'''
This script will do temporal PCA on given graph. 
...
all_path_dic.pop(4)
all_path_dic.pop(6)
save_path = r'D:\_GoogleDrive_Files\#Figs\#250211_Revision1\Fig4'
...
ax.set_yticklabels([0,2,4,6],fontsize = fontsize)
ax.set_xticks([0,4,9,14,19])
ax.set_xticklabels([1,5,10,15,20],fontsize = fontsize)

fig.savefig(ot.join(save_path,'Fig4B_Cell_PCA.png'),bbox_inches='tight')

#%%
'''
Fig 4C & 4D, Vector plotting on graph
'''
# Useful Functions
...
ax.set_zticks([])
fig.tight_layout()
fig.savefig(ot.join(save_path,'Fig4C_OD_Cell_PC_LE_Vec.png'),bbox_inches='tight')
#%% plot Orien vector graph.
# Fig 4C
...
ax.set_zticks([])
fig.tight_layout()
fig.savefig(ot.join(save_path,'Fig4D_Orien_Cell_PC_Vec.png'),bbox_inches='tight')
