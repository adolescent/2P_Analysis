...
all_path_dic = list(ot.Get_Sub_Folders(r'D:\#Fig_Data\_All_Spon_Data_V2'))
wp=r'D:\_GoogleDrive_Files\#Figs\#250211_Revision1\Fig5'
...
for i,cloc in tqdm(enumerate(all_path_dic)):
    ...
    all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Color',g16_events,g16_frames,'Real_Data']
    for j in tqdm(range(N_shuffle)):
        ...
        # save shuffle 
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Color',g16_frames_phase_s,g16_events_phase_s,'Phase_Shuffle']
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Color',g16_frames_dim_s,g16_events_dim_s,'Dim_Shuffle']
...
# ot.Save_Variable(wp,'Orien_Repeat_Similarity_V2',all_repeat_similarity)
# ot.Save_Variable(wp,'Orien_Repeat_Freq_V2',all_repeat_similarity)

ot.Save_Variable(wp,'Color_Repeat_Similarity_V2',all_repeat_similarity)
ot.Save_Variable(wp,'Color_Repeat_Freq_V2',all_repeat_similarity)
