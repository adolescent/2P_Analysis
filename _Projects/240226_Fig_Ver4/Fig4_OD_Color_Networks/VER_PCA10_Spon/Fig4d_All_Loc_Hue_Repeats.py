'''
Similar results as we did on spon.
'''
...
for i,cloc in tqdm(enumerate(all_path_dic)):
    ...
    hue_frames = (spon_label>0).sum()/len(spon_label)
    hue_events = Event_Counter(spon_label>0)*1.301/len(spon_label)
    all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Hue',hue_events,hue_frames,'Real_Data']
    for j in tqdm(range(N_shuffle)):
        ...
        hue_frames_phase_s = (spon_s_phase_label>0).sum()/len(spon_label)
        hue_events_phase_s = Event_Counter(spon_s_phase_label>0)*1.301/len(spon_label)
        hue_frames_dim_s = (spon_s_dim_label>0).sum()/len(spon_label)
        hue_events_dim_s = Event_Counter(spon_s_dim_label>0)*1.301/len(spon_label)
        # save shuffle 
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Hue',hue_frames_phase_s,hue_events_phase_s,'Phase_Shuffle']
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'Hue',hue_frames_dim_s,hue_events_dim_s,'Dim_Shuffle']
...
ot.Save_Variable(r'D:\_Path_For_Figs\240228_Figs_v4\Fig2\VerA_Direct_Spon_PCA','Hue_Repeat_Similarity',all_repeat_similarity)
ot.Save_Variable(r'D:\_Path_For_Figs\240228_Figs_v4\Fig2\VerA_Direct_Spon_PCA','Hue_Repeat_Freq',all_repeat_freq)
