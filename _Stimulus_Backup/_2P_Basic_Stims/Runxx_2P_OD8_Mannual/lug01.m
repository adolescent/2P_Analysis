...
Tpre=1.5;       % second, pre-stimulus blcrsinitank time, only for anesthetized imaging (runmode=3)
Tcue=3.5;      % Grating 1/2 duration in second (if Tcueb ~= 0, Tcue1 usually for adaption)
ISI = 3;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Nblock = 100;
stimname = 'Runxx_OD8_manual_';
...
%% prepare
    if flagstimprep   % this is an preparation, not for presentation
        ...
        if newstim==0
            fprintf('BLANK: stim %d ...', newstim);
        else if mod(newstim,2)==0 
        fprintf('ÓÒÑÛ´Ì¼¤£ºstim %d ...', newstim);
             else
            fprintf('×óÑÛ´Ì¼¤£ºstim %d ...', newstim);
            end
        end
        testrun=0;
