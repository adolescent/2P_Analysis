'''
The same plot as that in Fig 4 EFG, but on V2.
'''
...
wp = r'G:\我的云端硬盘\#Figs\#250211_Revision1\Fig5'
all_pair_corrs = ot.Load_Variable(wp,'All_Pair_Corrs_V2.pkl')
#%%
#%%
'''
Fig 5 Supp, we average all locations, and return the good ratio of pairwise correlation.
Ignore OD parts in V1.
'''
...
plt.show()
fig.savefig(ot.join(wp,'Fig5_Pairwise_Corr_Bars.png'),bbox_inches='tight')
...
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(11,5),dpi = 300)
...
for i,c_map in enumerate([heatmap_data_orien,heatmap_data_dist]):
...
fig.savefig(ot.join(wp,'Fig5_Pair_Corr.png'),bbox_inches='tight')