...
savepath = r'D:\_Path_For_Figs\240520_Figs_ver_F1\Fig2_PCA_SVM_Orien'
...
for i in range(10):
    ...
    sns.heatmap(c_pc_graph,center = 0,xticklabels=False,yticklabels=False,ax = axes[i//5,i%5],vmax = value_max,vmin = value_min,cbar_ax= cbar_ax,square=True,cmap = 'viridis')
...
#%%
'''
Fig 2d, we compare PC2 with HV and PC5 with AO, here we get the most similar maps.
'''
...
cbar_ax = fig.add_axes([0.96, .45, .03, .2])
sns.heatmap(ac.Generate_Weighted_Cell(spon_pcs[1,:]),center = 0,xticklabels=False,yticklabels=False,ax = axes[0],vmax = value_max,vmin = value_min,cbar_ax= cbar_ax,square=True,cmap = 'viridis')
sns.heatmap(ac.Generate_Weighted_Cell(spon_pcs[4,:]),center = 0,xticklabels=False,yticklabels=False,ax = axes[1],vmax = value_max,vmin = value_min,cbar_ax= cbar_ax,square=True,cmap = 'viridis')
...
fig,axes = plt.subplots(nrows=2, ncols=1,figsize = (4,7),dpi = 180)
cbar_ax = fig.add_axes([0.96, .45, .03, .2])
sns.heatmap(hv_map,center = 0,xticklabels=False,yticklabels=False,ax = axes[0],vmax = value_max,vmin = value_min,cbar_ax= cbar_ax,square=True)
sns.heatmap(ao_map,center = 0,xticklabels=False,yticklabels=False,ax = axes[1],vmax = value_max,vmin = value_min,cbar_ax= cbar_ax,square=True)
...
#%%
'''
Figs 2e, we compare top 10 PCs with all 6 functional maps, and we get similars.
Figs S2e is the same, just change spon_pcs into spon_pcs_s
'''
# get all response
...
sns.heatmap(all_corrs,center = 0,annot=True, fmt=".3f",ax = ax,vmax = value_max,vmin = value_min,cbar=False)
plt.show()
