...
#%%
'''
Step 0, generate corr and variation matrix. 
BOTH real data and shuffle included here, so for shuffle, it can be load in directly too.
'''
...
for i,cloc in enumerate(all_path_dic):
    ...
    for l in tqdm(range(N_shuffle)):
        spon_s = Spon_Shuffler(spon_frame=spon_series,filter_para=(0.005,0.65))
...
#%%
'''
Fig 2C , explained VAR of top 10 PC, with error bar on it.
...
fig.savefig(ot.join(savepath,'Fig2C_Explained_VAR.png'), bbox_inches='tight')
...
fig.savefig(ot.join(savepath,'FIg2G_All_PC_Corr.png'), bbox_inches='tight')