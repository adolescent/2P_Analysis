'''
This script stats for all orientation repeats at all data points. Making a Heatmap and a radar map.
...
#%%############################### Step1, Generate All Spon Corr Matrix. ######################################
all_spon_corr_mat = {}
for i,cloc in tqdm(enumerate(all_path_dic)):
    ...
    c_corr_mat = pd.DataFrame(0.0,columns = c_stim_maps.columns,index = range(len(c_spon)))
    for j in tqdm(range(len(c_spon))):
        single_spon = c_spon[j,:]
    ...
    all_spon_corr_mat[cloc_name] = c_corr_mat

ot.Save_Variable(work_path,'VAR3_All_Loc_Corr_Matrix',all_spon_corr_mat)
...
for i,cloc in tqdm(enumerate(all_path_dic)):
    ...
    analyzer = Classify_Analyzer(ac = ac,umap_model=c_model,spon_frame=c_spon,od = 0,orien = 1,color = 0,isi = True)
