...
#%%
'''
Fig 3A/B/C , example of PCA embeddings, Orientation.
NOTE PC has been changed, so be ware of annotation.
...
def Plot_Colorized_Oriens(axes,embeddings,labels,pcs=[4,1,2],color_sets = np.zeros(shape = (8,3))):
    ...
    axes.scatter3D(rest[:,0],rest[:,1],rest[:,2],s = 10,c = [0.7,0.7,0.7],alpha = 0.25,lw=0)
    axes.scatter3D(orien[:,0],orien[:,1],orien[:,2],s = 10,c=orien_colors,lw=0)
    return axes


#%% P1 Plot Orientation color bar here.
...
ax.set_zticks([])
fig.tight_layout()
# fig.savefig(ot.join(savepath,'Fig3B_Orien_Stim_PC234.png'),bbox_inches='tight')
# fig.savefig(ot.join(savepath,'Fig3C_Spon_Classified_PC234.png'),bbox_inches='tight')
# fig.savefig(ot.join(savepath,'Fig3A_Spon_Raw_PC234.png'),bbox_inches='tight')

#%%
'''
Fig 3 D/E recovered graph and correlation matrix.
Plot stim and spon seperately.

'''
value_max = 2
value_min = -1
fig_bar,ax_bar = Cbar_Generate(vmin = value_min,vmax = value_max,aspect=5,cmap=None,labelsize=10)
...
fig.tight_layout()
# fig.savefig(ot.join(savepath,'Fig3E_SponOrien_H.png'),bbox_inches='tight')
fig.savefig(ot.join(savepath,'Fig3D_StimOrien_H.png'),bbox_inches='tight')
#%% print R values here.
