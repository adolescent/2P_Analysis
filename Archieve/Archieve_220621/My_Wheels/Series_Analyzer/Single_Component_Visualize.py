...
def Single_Comp_Visualize(all_cell_dic,input_frame,shape = (512,512)):
    ...
    for i,cc in enumerate(used_cell_name):
        ...
        graph[y_list,x_list] = input_frame.loc[cc]
    sns.heatmap(graph,center = 0,square = 1, xticklabels=False, yticklabels=False)
    return graph

def Single_Mask_Visualize(all_cell_dic,cell_lists,shape = (512,512)):
    
    mask = pd.DataFrame(index = cell_lists)
    mask['Value'] = 1
    graph = Single_Comp_Visualize(all_cell_dic, mask,shape)
    return graph