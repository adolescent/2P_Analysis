...
def Tremble_Calculator_From_File(
    ...
    move_method : ('average'or'former'or'input'), optional
        Method of bais calculation. The default is 'average'. 
    ...
    for i in range(len(all_tif_name)):
        ...
        _,_,_,cutted_current_graph = Graph_Cutter(current_graph,boulder,cut_shape)
    ...
    if move_method == 'input':
        ...
        mass_center_maps = []
        for i in range(len(tremble_information[0])):# Cycle all 
            fig = plt.figure()
            ax = plt.subplot()
