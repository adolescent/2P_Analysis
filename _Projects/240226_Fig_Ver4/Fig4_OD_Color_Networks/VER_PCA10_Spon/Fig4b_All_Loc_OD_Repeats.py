'''
The same method as 2c used.
Generate OD repeat freq. and similariy.
...
for i,cloc in tqdm(enumerate(all_path_dic)):
    ...
    analyzer = Classify_Analyzer(ac = ac,umap_model=spon_models,spon_frame=c_spon,od = 1, color = 0,orien = 0)
    analyzer.Train_SVM_Classifier()
    # similarity calculator
    analyzer.Similarity_Compare_Average(od = 1,color = 0,orien = 0)
    ...
    od_frames = (spon_label>0).sum()/len(spon_label)
    od_events = Event_Counter(spon_label>0)*1.301/len(spon_label)
    all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'OD',od_events,od_frames,'Real_Data']
    for j in tqdm(range(N_shuffle)):
        ...
        od_frames_phase_s = (spon_s_phase_label>0).sum()/len(spon_label)
        od_events_phase_s = Event_Counter(spon_s_phase_label>0)*1.301/len(spon_label)
        od_frames_dim_s = (spon_s_dim_label>0).sum()/len(spon_label)
        od_events_dim_s = Event_Counter(spon_s_dim_label>0)*1.301/len(spon_label)
        # save shuffle 
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'OD',od_frames_phase_s,od_events_phase_s,'Phase_Shuffle']
        all_repeat_freq.loc[len(all_repeat_freq),:] = [cloc_name,'OD',od_frames_dim_s,od_events_dim_s,'Dim_Shuffle']

#%% ###############################FIG 4C , Plot repeat similarity #############################
plt.clf()
plt.cla()
# set graph
...
ot.Save_Variable(r'D:\_Path_For_Figs\240228_Figs_v4\Fig2\VerA_Direct_Spon_PCA','OD_Repeat_Similarity',all_repeat_similarity)
ot.Save_Variable(r'D:\_Path_For_Figs\240228_Figs_v4\Fig2\VerA_Direct_Spon_PCA','OD_Repeat_Freq',all_repeat_freq)