...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Nstim=4;        % total number of stimulus, used for generate stim parameter vectors (e.g. fixcolor, orientation...)
Ncond=5;        % Ncond = Nstim + number of blanks
Tpre=0;       % second, pre-stimulus blcrsinitank time, only for anesthetized imaging (runmode=3)
Tcue=2;      % Grating 1/2 duration in second (if Tcueb ~= 0, Tcue1 usually for adaption)
ISI = 3;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Nblock = 100;
stimname = 'Runxx_RGLum4_';
...
winsize1= [40 30];	 % window size in x and y in deg. For 20 inch screen at 570mm, [40, 30] is about full screen
gratingtype1=[2];  % 1: square wave, 2: sinewave, 3: load&display image
orientation1=[45 135 45 135]';   % 0-180, use 999 for random, orientation 0 is H gratings move up, 90 is Vertical gratings moving left
SF1=[0.15];           % Spatial frequency
velocity1=[0.5]';          % in cycle/sec
dutycycle1=[0.5]';          % white line is thiner if dutycycle < 0.5

rmax=11.45;     % update at 2019-04-11 DELL LCD 2P/electr desk
gmax=37.61;
bmax=3.55;
...
whitecolor2=[            % [R1 G1 B1; R2 G2 B2]  so whitecolor(1, :)=[R1 G1 B1]
1 0 0
1 0 0
rwratio rwratio rwratio 
rwratio rwratio rwratio 
...
backgroundcolor=(whitecolor1(3,:)+blackcolor1(3,:))./2;
...
orientation1b=[0]';  
orientation2b=[0]';   
direction1=[999]';            % 3 values allowed: 1, -1, 999; -1 is reverse direction (+180), 999 is random (either 1 or -1).
direction2=[999]';			% dir only use ful for random dir, other cases can be handeled by ori with range (0-360)
direction1b=[999];
direction2b=[999];
